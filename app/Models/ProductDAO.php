...
class ProductDAO
{
  ...
  {
    $prods = DB::table('Product')->join('Category', 'Product.CatID', '=','Category.ID')
                                 ->select('Product.*', 'Category.CatName')->get();
    return $prods;
  }
  ...
  static function selectByCateID($cateid) {
    $prods = DB::table('Product')->where('CatID', $cateid)->leftJOin->get();
    return $prods;
  }
